...
describe('day 10 challenge', ()=> {
    ...
    const isCorrupted = (line) => {        
        return isCorruptedDigit(line, 0, [])
    }
    const isCorruptedDigit = (line, index, expectedClosing) => {
        if (index == line.length) { return false; }
        
        let tag = line[index];
        if (isOpening(tag)) { expectedClosing.push(closingOf(tag)) }
        if (isClosing(tag) && tag != expectedClosing.pop()) { return tag; }
        
        return isCorruptedDigit(line, index + 1, expectedClosing);
    }
    const tags = [

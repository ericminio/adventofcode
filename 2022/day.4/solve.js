...
const pattern = /^(.*)-(.*),(.*)-(.*)$/;
const parse = (line, pattern) => pattern.exec(line).splice(1).map(item => parseInt(item));
...
const fullyContained = file => lines(file).map(line => contained(build(parse(line, pattern))));

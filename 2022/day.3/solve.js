...
const duplicate = (one, two) => { for (var i = 0; i < one.length; i++) { if (two[i] === one[i]) { return 'p'; } } return 'p' };
const errorIn = (line) => duplicate(line.substring(0, line.length), line.substring(line.length / 2));

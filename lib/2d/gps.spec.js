...
describe.only('gps', () => {
    ...
    `, function() {
        ...
        const generationN = (n, heads, map, request) => {
            let nexts = heads;
            for (var i = 0; i < n; i++) {
                nexts = nextGeneration(nexts, map, request);
            }
            return nexts;
        }

        it('explores as expected heads +1', () => {
            let nexts = generationN(1, heads, map, request);

            expect(Object.keys(nexts)).to.deep.equal(['1x2', '0x1']);
        });
        it('explores as expected heads +2', () => {
            let nexts = generationN(2, heads, map, request);

            expect(Object.keys(nexts)).to.deep.equal(['1x2', '1x1', '0x0']);
        });
        it('explores as expected heads +3', () => {
            let nexts = generationN(3, heads, map, request);

            expect(Object.keys(nexts)).to.deep.equal(['1x2', '1x0']);
        });
        it.skip('stops after heads +4', () => {
            let nexts = generationN(4, nexts, map, request);

            expect(Object.keys(nexts)).to.deep.equal(['1x2']);
        });
    });    
});

const gps = (request, map) => {
    let heads = init(request, map);
    let destination = search(request, map, heads);
    
    return path(destination);
};

const search = (request, map, heads) => {
    if (arrivedOn(request.target, heads)) { return Object.values(heads)[0]; }
    
    return search(request, map, nextGeneration(heads, map, request));
};

const nextGeneration = (heads, map, request) => {
    let nextHeads = {};
